...
const addChallengeForTraineeByTrainer = catchAsyncError(async (req, res) => {
    ...
    if (existingChallenge) {
        return res.status(409).json({
            success: false,
            message: "This trainee already has a challenge with this title. Please choose a different title.",
        });
    }
...
const updateChallengeForTraineeByTrainer = catchAsyncError(async (req, res) => {
    const { traineeId, challengeId } = req.params;
    const trainerId = req.user.payload.id;  
...
const deleteChallengeForTraineeByTrainer = catchAsyncError(async (req, res) => {
    ...
    if (!challenge) {
        return res.status(404).json({
            success: false,
            message: "Challenge not found or you do not have permission to delete this challenge.",
        });
    }
...
const getChallengesForTraineeByTrainer = catchAsyncError(async (req, res) => {
    ...
    const challenges = await Challenge.find({ trainee: traineeId, createdBy: trainerId });
